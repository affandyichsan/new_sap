...
if (count($reconcile) > 0) {
    ...
    $recKta     = ActionReconcile::akumulasiReconcile($data['note_per_date'], $total['roster'], $data['kta_n']);
    $recTta     = ActionReconcile::akumulasiReconcile($data['note_per_date'], $total['roster'], $data['tta_n']);
    $recIns     = ActionReconcile::akumulasiReconcile($data['note_per_date'], $total['roster'], $data['ins_n']);
    $recCC      = ActionReconcile::akumulasiReconcile($data['note_per_date'], $total['roster'], $data['cc_n']);
    $recWcu     = ActionReconcile::akumulasiReconcile($data['note_per_date'], $total['roster'], $data['wuc_n']);
    $recSmeet   = ActionReconcile::akumulasiReconcile($data['note_per_date'], $total['roster'], $data['s_meet_n']);
    if (count(json_decode($data['opk_detail'])) != 0) {
        $opkAc          = (isset($total['sap']['opk'])) ? ($data['opk_a'] + $total['sap']['opk']) : $data['opk_a'];
        @$recOpkobsA    = ActionReconcile::akumulasiReconcile($data['note_per_date'], $total['roster'], $data['opk_n']);
        $opkobsAch      = min(($opkAc / $recOpkobsA) * 100, 100);
    } else {
        $obsAc          = (isset($total['sap']['obs'])) ? ($data['obs_a'] + $total['sap']['obs']) : $data['obs_a'];
        @$recOpkobsA    = ActionReconcile::akumulasiReconcile($data['note_per_date'], $total['roster'], $data['obs_n']);
